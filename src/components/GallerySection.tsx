...
const GallerySection = () => {
  return (
    <section id="gallery" className="py-28 md:py-36 overflow-hidden">
      <div className="container">
        <ScrollReveal variant="tilt">
          <div className="text-center max-w-lg mx-auto mb-16">
            <div className="gold-line mx-auto mb-6" />
            <p className="text-gold font-semibold text-xs uppercase tracking-[0.25em] mb-4">Gallery</p>
        ...
        <p className="text-xs font-semibold text-gold uppercase tracking-[0.2em] mb-6">Stockyard</p>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 mb-16">
          {stockyardImages.map((img, i) => (
            <ScrollReveal key={img.label} variant="scale" delay={i * 0.04}>
              <div className="group relative overflow-hidden rounded-2xl aspect-[3/4] shadow-premium">
                <img
                  src={img.src}
                  alt={img.alt}
                  loading="lazy"
                ...
                <p className="absolute bottom-4 left-4 text-white text-sm font-heading font-semibold opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                  {img.label}
                </p>
              </div>
            </ScrollReveal>
        ...
        <p className="text-xs font-semibold text-gold uppercase tracking-[0.2em] mb-6">Rolling Mills</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
          {rollingMillImages.map((img) => (
            <ScrollReveal key={img.label} variant="card">
              <div className="relative overflow-hidden rounded-2xl aspect-video bg-muted flex items-center justify-center border border-dashed border-gold/20">
                {img.src ? (
                  <img src={img.src} alt={img.alt} loading="lazy" className="w-full h-full object-cover" />
                ) : (
                  <div className="text-center p-6">
                    <p className="text-muted-foreground text-sm font-heading font-semibold">{img.label}</p>
                    <p className="text-muted-foreground/50 text-xs mt-1">Photo coming soon</p>
                  </div>
                )}
              </div>
            </ScrollReveal>
